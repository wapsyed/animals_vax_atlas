...
legs_perc = gsea_btm_results_core_joined %>%
  ...
  group_by(pathogen, timepoint_comparison, process, status) %>%
...
df_legs <- dge_btm_process_genes_wide %>%
  ...
  inner_join(
    legs_perc %>%
      # filter(status == "Shared") %>%
      rename(n_status_perc = percent),
    by = join_by(pathogen, process, timepoint_comparison, status)
  ) %>%
  ...
  inner_join(
    dge_btm_gsea_mean_wide %>%
      ungroup() %>%
      # filter(process == process_selected)
      select(process, pathogen, timepoint_comparison, starts_with("nes_")),
  ...
  group_by(pathogen, process, timepoint_comparison) %>%
