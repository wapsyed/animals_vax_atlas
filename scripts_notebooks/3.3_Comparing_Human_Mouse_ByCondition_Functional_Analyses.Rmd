...
shared_processes_bypathogen_time = shared_perc_plot_df %>%
  ...
  # Scales
  scale_y_continuous(expand = c(0.05, 0.05)) +
  scale_fill_manual(values = btm_immune_groups$group) +
  guides(
    fill = guide_legend(
      byrow = TRUE,
      ncol = 1, 
...
legs_perc_barplot %>%
ggsave(filename = here("Figures", paste0("all_human_mouse_gsea_btm_results_leadingedge_core_barplot_selected_processes_filtered.png")),
         width = 8,
         height = 2.5)
...
legs_scatterplot_rank_significance = df_legs %>%
  ...
  labs(
       x = "Mouse Rank (Log2FC x -logP)",
       y = "Human Rank (Log2FC x -logP)",
       fill = "Status") +
 
  # Facets
  facet_grid(facet_label_vertical~.,
             axes = "all") +
...
legs_scatterplot_rank_significance %>% 
  ggsave(filename = here("Figures", "all_human_mouse_gsea_btm_results_leadingedge_core_ranks_scatterplot.png"),
         width = 4, height =8 )


#Labelled genes
legs_scatterplot_rank_significance_label = legs_scatterplot_rank_significance +
  geom_text_repel(aes(label = symbol),
                  data = . %>% filter(status == "Shared"),
                  size = 2, max.overlaps = 10000)  


legs_scatterplot_rank_significance_label

legs_scatterplot_rank_significance_label %>% 
  ggsave(filename = here("Figures", "all_human_mouse_gsea_btm_results_leadingedge_core_ranks_scatterplot_LabelledGenes.png"),
         width = 10, height = 4)
```
...
legs_scatterplot_rank_significance_horizontal = df_legs %>%
  ...
  # Facets
  facet_nested_wrap(treatment+facet_label_vertical~.,
              scales = "free",
             nrow = 1,
             strip = strip_nested(size = "variable")) +
...
legs_scatterplot_rank_significance_horizontal %>%
  ggsave(filename = here("Figures", "all_human_mouse_gsea_btm_results_leadingedge_core_ranks_scatterplot_horizontal.png"),
         width = 8, height = 3 )
...
dge_btm_process_genes_diff_bygene_clean = dge_btm_process_genes_diff_bygene %>% 
  ...
  select(treatment = treatment_human, pathogen = pathogen_human, timepoint_comparison, hgnc_symbol = human_symbol, mean_log2fc_mouse,
...
dge_btm_process_genes_diff_bygene_clean_filtered = 
  ...
  inner_join(
    gsea_btm_legs_clean %>%
      select(hgnc_symbol, pathogen, status, timepoint_comparison) %>% distinct(),
...
dge_btm_process_genes_diff_bygene_clean_filtered %>% dplyr::count(pathogen, timepoint_comparison, status)
...
legs_error_plot_pooled = dge_btm_process_genes_diff_bygene_clean_filtered %>% 
  filter(status %in% c("Shared", "Not-shared"),
         treatment %in% c("Infection", "Injury")) %>%
  ggplot() +
  aes(x = status,
...
#Plot
legs_error_plot_pooled

#Save
legs_error_plot_pooled %>%
  ggsave(filename = here("Figures", "all_human_mouse_legs_error_plot_pooled.png"),
         height = 3, width = 3)
...
dge_btm_process_genes_diff_bygene_process = dge_btm_process_genes_diff_bygene_clean_filtered %>% 
  full_join(btm_annotation_genes %>% 
  select(process, hgnc_symbol = symbol, group, subgroup),
  by = "hgnc_symbol") %>% 
