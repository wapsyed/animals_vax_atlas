...
# Pooled
dge_btm_process_genes_diff_bygene_process_pooled_plot = dge_btm_process_genes_diff_bygene_process %>% 
  filter(status %in% c("All genes"),
         treatment %in% c("Infection", "Injury")) %>%
  mutate(group = fct_relevel(group, immune_order) %>% fct_rev()) %>% 
  select(-process) %>% 
  distinct() %>% 
  ggplot() +
