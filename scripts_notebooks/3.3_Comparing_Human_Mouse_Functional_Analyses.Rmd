...
dge_btm_process_genes_diff_bygene_clean = dge_btm_process_genes_diff_bygene %>% 
  ...
  select(treatment = treatment_human, pathogen = pathogen_human, timepoint_comparison, hgnc_symbol = human_symbol, mean_log2fc_mouse, mean_log2fc_diff, se_mouse, p_adj_mouse) %>% 
...
gsea_btm_legs_clean =
  ...
  distinct() %>% 
  select(pathogen, process, group, timepoint_comparison, hgnc_symbol = gene, p_adjust_mouse_gsea, status) %>% 
  distinct() %>% 
...
dge_btm_process_genes_diff_bygene_clean_filtered = 
  ...
  inner_join(
    gsea_btm_legs_clean %>%
      select(hgnc_symbol, process, group, pathogen, p_adjust_mouse_gsea, status_original, status, timepoint_comparison) %>% distinct(),
